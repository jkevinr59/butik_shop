  ...
  <div class="hipsum">
    <div class="jumbotron">
      <table class="table table-bordered">
            <tr>
              <th>On Slider</th>
              <th>Ganti isi slider</th>
            </tr>
              <?php
                foreach($slider as $row)
                {
                  echo "<tr>";
        				  echo "<td><img src='".base_url('resource/').$row->foto."' width=200px>&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp $row->barang_nama</td>";
                  echo "<td>";
                  echo form_open('Admin/gantislider');
                  echo form_hidden('id', $row->id_barang);
                  ?>
                  <select name="new_barang">
                    <?php
                      foreach ($barang as $key) {
                        ?>
                          <option value="<?php echo $key->barang_id;?>"><?php echo $key->barang_nama;?></option>
                        <?php
                      }
                    ?>
                  </select>
                  <?php
                  echo form_submit('name', 'Ganti');
                  echo form_close();
                  echo "</td>";
        				  echo "</tr>";
                }
